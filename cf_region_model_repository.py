...
class CFRegionModelRepository(interfaces.RegionModelRepository):
    ...
    def cell_data_to_netcdf(self, region_model, output_dir):
        ...
        variables = {'y': [np.float, ('cell',), {'axis': 'mid_Y',
                                                 'units': 'm',
                                                 'standard_name': 'projection_y_coordinate'}],

                     'x': [np.float, ('cell',), {'axis': 'mid_X',
                                                 'units': 'm',
                                                 'standard_name': 'projection_x_coordinate'}],

                     'z': [np.float, ('cell',), {'axis': 'mid_Z',
                                                 'units': 'm',
                     ...
                     'catchment_id': [np.int32, ('cell',), {'grid_mapping': 'crs',
                                                            'units': '-',
                                                            'coordinates': 'y x z'}],
                     'x0': [np.float, ('cell',), {'axis': 'x0',
                                                  'units': 'm',
                                                  'standard_name': 'x0'}],

                     'y0': [np.float, ('cell',), {'axis': 'y0',
                                                  'units': 'm',
                                                  'standard_name': 'y0'}],

                     'z0': [np.float, ('cell',), {'axis': 'z0',
                                                  'units': 'm',
                                                  'standard_name': 'height0',
                                                  'long_name': 'z0'}],
                     'x1': [np.float, ('cell',), {'axis': 'x1',
                                                  'units': 'm',
                                                  'standard_name': 'x1'}],

                     'y1': [np.float, ('cell',), {'axis': 'y1',
                                                  'units': 'm',
                                                  'standard_name': 'y1'}],

                     'z1': [np.float, ('cell',), {'axis': 'z1',
                                                  'units': 'm',
                                                  'standard_name': 'height1',
                                                  'long_name': 'z1'}],
                     'x2': [np.float, ('cell',), {'axis': 'x2',
                                                  'units': 'm',
                                                  'standard_name': 'x2'}],

                     'y2': [np.float, ('cell',), {'axis': 'y2',
                                                  'units': 'm',
                                                  'standard_name': 'y2'}],

                     'z2': [np.float, ('cell',), {'axis': 'z2',
                                                  'units': 'm',
                                                  'standard_name': 'height2',
                                                  'long_name': 'z2'}],
                     'slopes': [np.float, ('cell',), {'units': 'deg',
                                                      'standard_name': 'slopes'}],
                     'aspects': [np.float, ('cell',), {'units': 'deg',
    ...
    def parse_tinrepo(self):
        from rasputin import tin_repository as tin  # due to linux/win compat, we take the import here
        # Construct cells from TIN repository
        print(Path(self._tin_data_folder))
        tin_repo = tin.TinRepository(path=Path(self._tin_data_folder))
        ...
        for tid in self._tin_uid_:
            ...
            info = tin_repo.info(uid=tid)
            cid_val = int(''.join(filter(str.isdigit, tid)))
            #print(geomdic)
            materials = [(int(np.round(r*255)),int(np.round(g*255)), int(np.round(b*255))) for (r,g,b) in geomdic.colors]
            pv = np.asarray(geomdic.points)
            fv = np.asarray(geomdic.faces)
            projstring = info["tin"]["projection"]
            #print(materials)
            land_cover_type = info["info"]["land_cover_type"]
            #print(land_cover_type)
            land_covers = info["info"]["land_covers"]
            #print(land_covers)
            for c in materials:
